...
export class AiProgram {
    constructor(pio, trans) {
        this.pio = pio;
        this._ais = Object.keys(pio.lists.param_root.lists.AI.lists).map(
            key => pio.lists.param_root.lists.AI.lists[key]
        );
        this._actions = Object.keys(pio.lists.param_root.lists.Action.lists).map(
            key => pio.lists.param_root.lists.Action.lists[key]
        );
        this._behaviors = Object.keys(pio.lists.param_root.lists.Behavior.lists).map(
            key => pio.lists.param_root.lists.Behavior.lists[key]
        );
        this._queries = Object.keys(pio.lists.param_root.lists.Query.lists).map(
            key => pio.lists.param_root.lists.Query.lists[key]
        );
    ...
    add_ai(ai) {
        if (ai.hasOwnProperty("_plist")) {
            this._ais.push(ai._plist);
        } else {
            this._ais.push(ai);
        }
    }
    ...
    get_item_parents(idx) {
        return idx > -1
            ? this.items.filter(
                  item => item.child_list && item.child_list.includes(idx)
              )
            : [];
    }
    ...
    get behaviors_offset() {
        return this._ais.length + this._actions.length;
    }
    get queries_offset() {
        return this._ais.length + this._actions.length + this._behaviors.length;
    }

    get_item_type_index(idx) {
        if (idx < this.actions_offset) {
            return `AI_${idx}`;
        } else if (idx < this.behaviors_offset) {
            return `Action_${idx - this.actions_offset}`;
        } else if (idx < this.queries_offset) {
            return `Behavior_${idx - this.behaviors_offset}`;
        } else return `Query_${idx - this.queries_offset}`;
    }

    find_item(needle) {
        for (let i = 0; i < this.items.length; i++) {
            if (
                (this.items[i].ClassName == needle.ClassName &&
                    !this.items[i].Name &&
                    !needle.Name) ||
                this.items[i].Name == needle.Name
            )
                return i;
        }
        return -1;
    ...
    async get_tree(trans_map) {
        return (await pywebview.api.get_ai_tree({ pio: this.pio })) || [];
    ...
    _create_ai_tree(idx, trans_map) {
        ...
        let tree = {
            text: try_trans(
                this.items[idx].Name || this.items[idx].ClassName,
                trans_map
            ),
            expanded: true,
            index: idx,
        ...
        if (ai._plist.objects.ChildIdx)
            tree.nodes = ai.child_list.map(i =>
                i >= 0 ? this._create_ai_tree(i, trans_map) : {}
            );
        return tree;
    }
...
export class AiItem {
    ...
    get SInst() {
        return this._plist.objects.SInst ? this._plist.objects.SInst.params : null;
    }
    get ChildIdx() {
        return this._plist.objects.ChildIdx
            ? this._plist.objects.ChildIdx.params
            : null;
    }
    get BehaviorIdx() {
        return this._plist.objects.BehaviorIdx
            ? this._plist.objects.BehaviorIdx.params
            : null;
    }
