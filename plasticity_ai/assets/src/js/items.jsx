...
import PlasticContext, { AiItemsList, ParamInput, NewAiModal } from './general.jsx';
...
export default class ProgramItemsView extends Component {
    ...
    class_names = () => {
        return Object.keys(this.props.classes).map(cls => <option key={cls} value={cls}>{cls}</option>);
    }

    async add_new_ai(def) {
        this.props.setLoading();
        const new_ai = await pywebview.api.create_prog_item({type: this.props.type, def});
    ...
    update_behavior(e, param) {
        let up_ai = new AiItem(this.state.selected._plist);
        up_ai._plist.objects.BehaviorIdx.params[param] = { int: parseInt(e.currentTarget.value.replace('Behavior_', '')) };
        this.setState({selected: up_ai, modified: true});
    }
    ...
    render() { 
        return (
            <div className="row">
                <button className={((!this.state.modified) ? 'fab-away ' : '' ) + "btn btn-lg btn-primary fab "}
                    onClick={this.push_update.bind(this)}>
                    <i className="material-icons">done</i>
                </button>
                <div className="prog_list">
                    <h2>AI List</h2>
                    <select id={`${this.props.type}_list`} size="50" className="items_list" onChange={this.ai_select_change}>
                        {this.props.items && <AiItemsList items={this.props.items} use_label={this.props.label_list} />}
                    </select>
                    <div className="list_buttons">
                        <button className="btn btn-sm btn-primary" onClick={() => $(`#new_${this.props.type}_modal`).modal('show')}>+</button>
                        <button className="btn btn-sm btn-primary" onClick={() => (confirm('Delete this item?')) ? this.remove_ai() : null}>‒</button>
                    </div>
                </div>
                <div className="info">
                    <div className="row">
                        <div className="main-col">
                            <div className="info-box">
                                ...
                                <table id={`${this.props.type}_def`} className="params">
                                    <tbody>
                                        <tr>
                                            <td><label htmlFor={`${this.props.type}_name`}>Name</label></td>
                                            <td>
                                                <input type="text" id={`${this.props.type}_name`}
                                                    value={(this.state.selected) ? this.state.selected.Name : ''}
                                                    onChange={(e) => this.update_name(e)} />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><label htmlFor={`${this.props.type}_class`}>ClassName</label></td>
                                            <td>
                                                <select id={`${this.props.type}_class`} readOnly={true}
                                                    value={(this.state.selected) ? this.state.selected.ClassName : ''}>
                                                        {(this.state.selected) ?  this.class_names() : ''}
                                                </select>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><label htmlFor={`${this.props.type}_group`}>GroupName</label></td>
                                            <td>
                                                <select type="text" id={`${this.props.type}_group`}
                                                    value={(this.state.selected) ? this.state.selected.GroupName : ''}
                                                    onChange={(e) => this.update_group(e)}>
                                                    <option value=''></option>
                                                    {this.context.aiprog.items.map((item) => {
                                                        return (item.Name)
                                                            ? <option key={item.Name + ((item.GroupName) ? item.GroupName : '')} value={item.Name}>
                                                                {try_trans(item.Name, this.context.trans)}
                                                            </option>
                                                            : ''
                                                    })}
                                                </select>
                            ...
                            {this.state.selected && this.state.selected.ChildIdx &&
                                <div className="info-box" id={`${this.props.type}_children`}>
                                    <h2>Children</h2>
                                    <table className="params" id={`${this.props.type}_childidx`}>
                                        <tbody>
                                            {Object.keys(this.state.selected.ChildIdx).map(param => {
                                                return (
                                                    <tr key={param}>
                            ...
                            {this.state.selected && this.state.selected.BehaviorIdx &&
                                    <div className="info-box" id={`${this.props.type}_behaviors`}>
                                        <h2>Behaviors</h2>
                                        <table className="params" id={`${this.props.type}_bidx`}>
                                            <tbody>
                                                {Object.keys(this.state.selected.BehaviorIdx).map(param => {
                                                    return (
                                                        <tr key={param}>
                                                            <td>{param}</td>
                                                            <td>
                                                                <select id={`${this.props.type}_behave_${param}`} onChange={(e) => this.update_behavior(e, param)}
                                                                value={Object.keys(this.context.aiprog.behaviors)[this.state.selected.BehaviorIdx[param].int]}>
                                                                    <AiItemsList items={this.context.aiprog.behaviors} />
                                                                </select>
                                                            </td>
                                                        </tr>
                                                    )
                            ...
                            <div className="main-col">
                                {this.state.selected && this.state.selected.SInst &&
                                    <div class='info-box' id={`${this.props.type}_sinst`}>
                                        <h2>Parameters</h2>
                                        <table className="params" id={`${this.props.type}_sparams`}>
                                            <tbody>
                                                {Object.keys(this.state.selected.SInst).map(param => {
                                                    return (
                                                        <tr key={param + this.state.selected.SInst[param].toString()}>
                                                            <td dangerouslySetInnerHTML={{__html: param}} />
                                                            <td>
                                                                <ParamInput param={param} value={this.state.selected.SInst[param]} 
                                                                    onChange={this.update_sinst.bind(this)}
                                                                />
                                                            </td>
                                                        </tr>
                                                    )
                                ...
                                {this.state.selected && 'DynamicInstParams' in this.props.classes[this.state.selected.ClassName] &&
                                    <div className="info-box" id={`${this.props.type}_dinst`}>
                                        ...
                                        <table className="table table-sm">
                                            ...
                                            <tbody>
                                                {this.props.classes[this.state.selected.ClassName].DynamicInstParams.map(dinst => {
                                                    return <tr key={dinst.Name}>
                                                        <td>{dinst.Name}</td><td>{dinst.Type}</td>
                                                    </tr>
                                                })}
                                            </tbody>
                                        </table>
