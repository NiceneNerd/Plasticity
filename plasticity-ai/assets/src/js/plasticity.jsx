...
export default class PlasticityRoot extends Component {
    ...
    async add_ai(type, new_ai) {
        ...
        this.setState({aiprog: new_prog, modified: true})
    }
    ...
    update_item(idx, new_item) {
        ...
        this.setState({aiprog: new AiProgram(new_pio), modified: true});
    }
    ...
    async open_file() {
        ...
        const open_ai = await pywebview.api.open_file();
        if (open_ai) {
    ...
    async save_file(path) {
        ...
        else this.setState({ modified: false, path });
    }
